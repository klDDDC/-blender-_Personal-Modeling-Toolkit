...
发布基础设施已补充：`tools/build_release.py` 会从 `bl_info` 读取版本并生成可复现 ZIP；`tools/install_addon.py` 会先分阶段解压和校验，再以继承 Blender 插件目录权限、失败可回滚的方式替换旧副本；`tools/推送.bat` 已改为调用本地安装流程，不再切换硬编码路径或直接推送 `main`。剩余发布风险是仓库仍缺少 CI 矩阵，且插件声明版本 `1.6.0` 与远端标签 `v1.2` 不一致。因此正式重大版本仍需先统一版本元数据、运行 CI 并由我在明确授权后提交和推送。

## 1.8.0 实施与复测（2026-08-02）

本节是对历史评估的追加结论，不修改上面的原始审查记录。1.8.0 已将材质交互和快捷键持久化纳入正式实现：

- 单物体材质、多物体独立材质和展开后的名称族成员色块均可打开材质操作弹窗；名称族标题仍只负责展开。替换与选面始终按材质数据块和库路径精确匹配，作用范围固定为当前选中的网格对象。
- 清理行为不再按 `.001/.002` 名称强制合并正在使用的材质。两个清理入口只删除确认属于重复族、用户计数为零的本地数字后缀材质，并保护 Fake User、Asset、链接库及任何真实引用。
- 快捷键使用版本化 JSON 和稳定 Operator 标识保存；自动监控包含键/鼠标类型、修饰键、激活状态与重复触发，带防抖、写盘失败重试、立即保存、重置和状态反馈。
- 插件版本、安装校验和发布包名称已统一到 `1.8.0`。Blender 4.1.1 源码与安装目录核心回归均为 `20/20`，UI/交互回归 `5/5`，快捷键隔离双进程重启测试 `2/2`，63 个材质 ID 预设、斜向拉直、布局分割和安装版法线专项均通过。发布 ZIP 两次构建 SHA-256 一致，安装目录 16 个源码文件与仓库逐文件一致；Git 推送结果在本次提交完成后记录。
